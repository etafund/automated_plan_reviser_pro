...
def base_result(provider: str, scenario: str, prompt_text: str) -> dict[str, Any]:
    ...
    result_id = f"provider-result-{config['provider_slot']}-{scenario}"
    result = {
...
def validate_provider_result_contract(result: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    forbidden_keys = {"raw_hidden_reasoning", "chain_of_thought", "reasoning_content"}
    present = sorted(key for key in forbidden_keys if key in result)
    if present:
    ...
    if result.get("provider_slot") == "deepseek_v4_pro_reasoning_search":
        ...
        if result.get("reasoning_effort") != "max":
            errors.append("DeepSeek result must use reasoning_effort=max")
        if result.get("reasoning_content_policy") != "transient_tool_replay_hash_only_persisted":
            errors.append("DeepSeek result must persist only reasoning_content hash")
        if not result.get("citations"):
            errors.append("DeepSeek result must include at least one citation")
    if result.get("provider_slot") == "xai_grok_reasoning":
        if result.get("reasoning_effort") != "high":
...
def validate_fixtures() -> tuple[bool, dict[str, Any], list[dict[str, str]], list[str]]:
    ...
    for provider in ("deepseek", "xai"):
        ok, result, errors = build_result(provider, "success", "fixture validation prompt")
        generated_ok = generated_ok and ok and not errors and not validate_provider_result_contract(result)
    if not generated_ok:
        failures.append("generated success results do not satisfy adapter contract")
    data = {
    ...
    warnings: list[str] = []
    if not jsonschema_errors(envelope(True, {}), JSON_ENVELOPE_SCHEMA):
        data["json_envelope_contract"] = "validated"
    else:
        warnings.append("jsonschema unavailable or envelope validation skipped")
    return not failures, data, errors, warnings


def prompt_text_from_args(args: argparse.Namespace) -> str:
    if args.prompt:
        try:
            return Path(args.prompt).read_text(encoding="utf-8")
        except OSError as exc:
            raise SystemExit(f"failed to read prompt {args.prompt}: {exc}") from exc
    return "mock v18 provider adapter prompt"


def main() -> int:
    parser = argparse.ArgumentParser(description="Deterministic v18 xAI and DeepSeek provider adapter harness.")
    parser.add_argument("--provider", choices=sorted(PROVIDER_CONFIGS), help="Provider route to exercise.")
    parser.add_argument("--scenario", choices=["success", *sorted(FAILURE_SCENARIOS)], default="success")
    parser.add_argument("--prompt", help="Prompt text file; omitted uses deterministic mock text.")
    parser.add_argument("--output", help="Optional path to write the provider_result artifact.")
    parser.add_argument("--check-env", action="store_true", help="Require the provider API key env var to be present.")
    parser.add_argument("--validate-fixtures", action="store_true", help="Validate success and negative adapter fixtures.")
    ...
    if args.validate_fixtures:
        ...
        out = envelope(
            ...
            errors=errors,
            warnings=warnings,
            blocked_reason=None if ok else "provider adapter fixture validation failed",
            next_command=None if ok else "python3 scripts/xai-deepseek-adapters.py --validate-fixtures --json",
            fix_command=None if ok else "fix listed provider adapter fixture violations",
        )
        print(json.dumps(out, indent=2, sort_keys=True) if args.json else ("ok" if ok else "\n".join(e["message"] for e in errors)))
        return 0 if ok else 1

    if not args.provider:
        message = "--provider is required unless --validate-fixtures is used"
        out = envelope(False, {}, errors=[{"error_code": "usage_error", "message": message}], blocked_reason="missing_provider")
        print(json.dumps(out, indent=2, sort_keys=True) if args.json else message, file=sys.stdout if args.json else sys.stderr)
        return 2
    ...
    if args.check_env and not env_present:
        ok, result, errors = build_result(args.provider, "auth_failure", prompt_text)
    else:
        ok, result, errors = build_result(args.provider, args.scenario, prompt_text)
    contract_errors = validate_provider_result_contract(result) if result else []
    if contract_errors:
        ok = False
        errors.extend({"error_code": "contract_violation", "message": error} for error in contract_errors)
    ...
    out = envelope(
        ...
        data,
        errors=errors,
        blocked_reason=None if ok else "provider adapter scenario failed",
        next_command=None if ok else "retry after resolving the provider adapter error",
        fix_command=None if ok else "inspect .data.provider_result.error",
        retry_safe=True,
    )
    if args.json:
        print(json.dumps(out, indent=2, sort_keys=True))
    elif ok:
        print(f"{args.provider} {args.scenario} provider result is synthesis-eligible")
    else:
        for error in errors:
