...
output_dir: rounds
EOF
    cat > .apr/workflows/solo.yaml <<EOF
readme: README.md
spec: SPEC.md
model: gpt-4
output_dir: rounds
EOF
    ...
    mkdir -p .apr/rounds/solo
    echo "Solo round content" > .apr/rounds/solo/round_1.md
...
@test "apr history desktop layout shows previews" {
    ...
    assert_output --partial "apr diff 1 2"
}
...
@test "apr show displays header and CTA" {
    ...
    refute_output --partial "apr diff 0 1"
}

@test "apr show displays valid diff CTA when previous round exists" {
    export APR_LAYOUT=desktop
    export NO_COLOR=1
    run "${PROJECT_ROOT}/apr" show 2 -w test
    assert_success
    assert_output --partial "ROUND 2: test"
    assert_output --partial "Round 2 content"
    assert_output --partial "-> Compare: apr diff 1 2"
}

@test "apr history omits impossible diff CTA when latest round is 1" {
    export APR_LAYOUT=desktop
    export NO_COLOR=1
    run "${PROJECT_ROOT}/apr" history -w solo
    assert_success
    assert_output --partial "REVISION HISTORY: solo"
    assert_output --partial "Round 1"
    assert_output --partial "Solo round content"
    refute_output --partial "apr diff 0 1"
